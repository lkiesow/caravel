...
**Done.** Migration `0004_dates_from_itinerary` in both dialects drops
`item_dates` and its index and adds `idx_itinerary_entries_item_id`, which
`ListItineraryDatesByItem` needs and which also helps the cascade when a
location is deleted. The rows are discarded, and the up migration says plainly
why: the two models had disagreed for as long as both existed, so any rule for
reconciling them would be a guess. The down migration recreates the table
*empty*, with a comment saying the data cannot come back and that inventing
`item_dates` rows from `itinerary_entries` would be worse than an empty table,
because it would look like the data survived.

`queries/item_dates.sql` is deleted, both dialects regenerated by hand, and
`CreateItemDate`/`ListItemDatesByItem`/`DeleteItemDate`, `CreateItemDateParams`
and the `ItemDate` domain type are gone from `store.go`, `domain.go` and both
store implementations. The seeder's `dateSpec` went with them: Kirkjufell is
already put on an itinerary day by the `full` scenario, so its Dates card now
renders with content for free -- the unification demonstrating itself, and one
fewer thing to seed. Both feature docs gained a paragraph saying the two are one
fact with two ways in.

Verified: `make ci` and `make test-postgres` green. The migration was
round-tripped up/down/up on a real database of **each** dialect with a
throwaway `golang-migrate` program: version 3 -> 4 drops `item_dates` and adds
the index, 4 -> 3 restores the table and drops the index, and 3 -> 4 again is
clean. A fresh `cmd/seed` run against an empty database produced 7 scenarios, no
`item_dates` table, the new index present, and Kirkjufell on one itinerary day.
`make docs` builds clean under `--strict`, so the two new cross-links resolve.

Two deviations:

- **Screenshots were regenerated and then reverted.** `location-detail.png` is
  the one capture showing a location's Dates card -- except it is unscrolled and
  stops at the Location card, so the card is not in frame. Regenerating left 14
  of 15 files byte-identical and `assistant.png` differing by a few hundred
  bytes of run-to-run noise, which is unrelated churn. Reverted. (The plan's
  "only if the seeder change visibly alters a captured page" turned out to mean
  no.)
- **`CLAUDE.md` gained a gotcha.** `sqlc generate` never deletes: removing
  `item_dates.sql` left both `gen/item_dates.sql.go` files behind, referring to
  types `models.go` had just dropped. Worse in the same vein, the orphaned
  `ItemDate` struct in `domain.go` survived a first removal attempt silently,
  because an unused exported type compiles fine -- `go build` cannot catch that
  one, so a grep after the removal is the check.
