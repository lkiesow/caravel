...
// no delete buttons, and disabled checkboxes — ticking a shared list is a write
// like any other. Same option name and shape as file-list.js's, so the two
// read-only surfaces are configured the same way.
export async function renderChecklistList(container, tripId, { readOnly = false } = {}) {
    ...
    for (const checklist of checklists) {
      ...
      card.innerHTML = `
        <div class="checklist-card__header">
          ...
          ${readOnly ? "" : `<button class="icon-remove" data-action="delete-checklist" aria-label="${t("common.delete")}">${icon("trash-2")}</button>`}
        </div>
      ...
      for (const item of checklist.items) {
        ...
        li.innerHTML = `
          ...
          ${readOnly ? "" : `<button class="icon-remove" data-action="delete-item" aria-label="${t("common.remove")}">${icon("x")}</button>`}
        `;
        if (!readOnly) {
