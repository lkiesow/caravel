...
//
// A measurement tool, not a spec: its output is numbers to read, and the pass/fail
// judgement usually needs a human (large text has a lower bar, decorative fills
// have none). Run it, read it, act on it.
//
...
//   node tests/ui/contrast.js --route /trips --scheme both --min 4.5
//
...
function parseArgs(argv) {
  const opts = { route: "/trips", scheme: "light", selectors: [], min: null, selfTest: false, url: process.env.CARAVEL_TEST_URL || "http://localhost:8080" };
...
async function measureScheme(browser, opts, scheme) {
  ...
  await page.goto(opts.url + opts.route);
  ...
  if (landed !== opts.route) {
    ...
    console.error(`contrast: asked for ${opts.route} but landed on ${landed} — that route probably doesn't exist`);
    process.exit(1);
  }
...
function report(scheme, results, min) {
  console.log(`\n=== ${scheme} — ${results.length} element(s) measured ===`);
  if (!results.length) {
  ...
  for (const r of results.sort((a, b) => a.ratio - b.ratio)) {
    ...
    const bgNote = r.translucentBg ? ` (${r.bgLayers} layers, flattened)` : "";
    console.log(
...
async function main() {
  ...
  try {
    for (const scheme of schemes) {
      const results = await measureScheme(browser, opts, scheme);
      report(scheme, results, opts.min);
